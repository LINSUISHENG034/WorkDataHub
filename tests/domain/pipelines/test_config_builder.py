...
class TestStepConfigValidation:
    ...
    def test_step_config_valid_configuration(self):
        ...
        assert config.import_path == "work_data_hub.domain.pipelines.adapters.CleansingRuleStep"
...
class TestDynamicImportFunctionality:
    ...
    def test_import_step_class_success(self):
        ...
        step_class = _import_step_class_or_factory("tests.domain.pipelines.conftest.UpperCaseStep")
    ...
    def test_import_step_class_class_not_found(self):
        ...
        with pytest.raises(PipelineAssemblyError, match="Could not import"):
            _import_step_class_or_factory("tests.domain.pipelines.conftest.NonexistentClass")
    ...
    def test_import_factory_method_success(self):
        ...
        factory = _import_step_class_or_factory("tests.domain.pipelines.conftest.MockTransformStep")
    ...
    def test_import_nonexistent_factory_method(self):
        ...
        with pytest.raises(PipelineAssemblyError, match="Could not import"):
            _import_step_class_or_factory("tests.domain.pipelines.conftest.UpperCaseStep.nonexistent_method")
    ...
    def test_create_step_instance_import_failure(self):
        ...
        config = StepConfig(
            name="failing_step",
            import_path="nonexistent.module.ClassName",
            options={}
        )

        with pytest.raises(PipelineAssemblyError, match="Failed to create step 'failing_step'"):
...
class TestPipelineBuilder:
    ...
    def test_pipeline_builder_build_without_config(self, mock_steps):
        ...
        with pytest.raises(PipelineAssemblyError, match="Pipeline configuration is required"):
            builder.build()
    ...
    def test_pipeline_builder_build_without_steps(self, sample_config):
        ...
        with pytest.raises(PipelineAssemblyError, match="Pipeline must have at least one step"):
            builder.build()
...
class TestBuildPipelineFunction:
    ...
    def test_build_pipeline_from_dict_config(self, sample_config_dict):
        ...
        with patch('src.work_data_hub.domain.pipelines.builder._create_step_instance') as mock_create:
    ...
    def test_build_pipeline_step_count_mismatch(self, sample_config_dict, mock_steps):
        ...
        with pytest.raises(PipelineAssemblyError, match="Step count mismatch"):
            build_pipeline(sample_config_dict, steps=mock_steps[:1])  # Config expects 2 steps
    ...
    def test_build_pipeline_invalid_config_dict(self):
        ...
        with pytest.raises(PipelineAssemblyError, match="Invalid pipeline configuration"):
