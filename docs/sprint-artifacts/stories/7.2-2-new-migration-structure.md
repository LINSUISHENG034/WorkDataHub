...
- [ ] **Task 0: Prerequisites (BLOCKED BY Story 7.2-3)**
  - [ ] Subtask 0.1: Verify Story 7.2-3 is complete (Domain Registry `id` unification)
  - [ ] Subtask 0.2: Create `config/seeds/` directory
  - [ ] Subtask 0.3: Export `company_types_classification` data to CSV (104 rows)
  - [ ] Subtask 0.4: Export `industrial_classification` data to CSV (1,183 rows)

- [ ] Task 1: Create `001_initial_infrastructure.py` (AC: #1) - **17 tables**
  - [ ] Subtask 1.1: Create migration file with revision `20251228_000001`
  - [ ] Subtask 1.2: Define public schema tables (pipeline_executions, data_quality_metrics)
  - [ ] Subtask 1.3: Define enterprise schema tables (base_info, business_info, biz_label, enrichment_requests, enrichment_index, validation_results)
  - [ ] Subtask 1.4: Define enterprise classification tables (company_types_classification, industrial_classification) - structure only
  - [ ] Subtask 1.5: Define mapping schema seed tables (产品线, 组织架构, 计划层规模)
  - [ ] Subtask 1.6: Define mapping reference tables (年金客户, 产品明细, 利润指标) - **手动 DDL，未注册域**
  - [ ] Subtask 1.7: Define system schema table (sync_state)
  - [ ] Subtask 1.8: Add idempotent table existence checks (`IF NOT EXISTS`)

- [ ] Task 2: Create `002_initial_domains.py` (AC: #2, #7) - **4 registered domains only**
  - [ ] Subtask 2.1: Create migration file with revision `20251228_000002`, down_revision to `20251228_000001`
  - [ ] Subtask 2.2: Use `ddl_generator.generate_create_table_sql("annuity_performance")` for business.规模明细
  - [ ] Subtask 2.3: Use `ddl_generator.generate_create_table_sql("annuity_income")` for business.收入明细
  - [ ] Subtask 2.4: Use `ddl_generator.generate_create_table_sql("annuity_plans")` for mapping.年金计划
  - [ ] Subtask 2.5: Use `ddl_generator.generate_create_table_sql("portfolio_plans")` for mapping.组合计划
  - [ ] Subtask 2.6: Add idempotent table existence checks

- [ ] Task 3: Create `003_seed_static_data.py` (AC: #3, #8)
  - [ ] Subtask 3.1: Create migration file with revision `20251228_000003`, down_revision to `20251228_000002`
  - [ ] Subtask 3.2: Load company_types_classification from CSV (104 rows)
  - [ ] Subtask 3.3: Load industrial_classification from CSV (1,183 rows)
  - [ ] Subtask 3.4: Embed seed data for 产品线 (12 rows)
  - [ ] Subtask 3.5: Embed seed data for 组织架构 (38 rows)
  - [ ] Subtask 3.6: Embed seed data for 计划层规模 (7 rows)
  - [ ] Subtask 3.7: Embed seed data for 产品明细 (18 rows)
  - [ ] Subtask 3.8: Embed seed data for 利润指标 (12 rows)
  - [ ] Subtask 3.9: Use `INSERT ... ON CONFLICT DO NOTHING` for idempotency

- [ ] Task 4: Verification and validation (AC: #4, #5, #6)
  - [ ] Subtask 4.1: Run `alembic history` - verify linear chain with 3 migrations
  - [ ] Subtask 4.2: Run `alembic upgrade head` on clean test database
  - [ ] Subtask 4.3: Verify all 21 tables created (14 infrastructure + 7 domains)
  - [ ] Subtask 4.4: Verify seed data populated (~1,350 rows total)
  - [ ] Subtask 4.5: Verify no migration heads (single linear chain)

- [ ] Task 5: Cross-validation with Domain Registry (AC: #7)
  - [ ] Subtask 5.1: Compare `ddl_generator` output with migration table definitions
  - [ ] Subtask 5.2: Verify annuity_performance table: columns, indexes, primary_key=`id`
  - [ ] Subtask 5.3: Verify annuity_income table: columns, indexes, primary_key=`id`
  - [ ] Subtask 5.4: Verify annuity_plans table: columns, indexes, primary_key=`id`
  - [ ] Subtask 5.5: Verify portfolio_plans table: columns, indexes, primary_key=`id`
...
### Seed Data (~1,350 rows) - Phase 3

**Large Datasets (CSV Source)**:
- `company_types_classification` - 104 rows (from `config/seeds/company_types_classification.csv`)
- `industrial_classification` - 1,183 rows (from `config/seeds/industrial_classification.csv`)

**Small Datasets (Embedded)**:
- `产品线` - 12 rows
- `组织架构` - 38 rows
- `计划层规模` - 7 rows
- `产品明细` - 18 rows
...
> **CRITICAL**: All seed data must be version-controlled in code repository. NO external database dependencies (including Legacy MySQL).
...
None (no code execution required for story creation)

### Completion Notes List

*Story not yet implemented - awaiting dev-story execution*

### File List

**Story Created**: 2025-12-28
**Status**: ready-for-dev

**Expected Artifacts** (to be created during implementation):
...
- `config/seeds/company_types_classification.csv` (if not exists)
- `config/seeds/industrial_classification.csv` (if not exists)
