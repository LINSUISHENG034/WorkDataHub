...
- [x] AC15: Parity validation checklist included（含可执行检查步骤）
- [x] AC16: `clean_company_name` regex logic explicitly documented（按 legacy 规则分点列出）

### Documentation & Template Requirements（执行即完成）
- 输出文件：`docs/cleansing-rules/annuity-income.md`（保持与 `docs/cleansing-rules/index.md` 链接一致）
- 模板：严格套用 `docs/templates/cleansing-rules-template.md`，填满所有表格/段落；不保留占位符
- 必填内容：
  - 1) Domain Overview：类名 `AnnuityIncomeCleaner`，Excel Sheet `收入明细`，目标表/Schema 写明（若缺请标注 TODO，但留可填位）
  - 2) Column Mappings：按执行顺序列 1-11（见 Cleansing Operations 表），含 Transformation/Notes
  - 3) Cleansing Rules：Rule ID（CR-001..）、Field、Rule Type、Logic、Priority、Notes；标注优先级（执行序）
  - 4) Company ID Resolution Strategy：标准优先级链；显式标注 COMPANY_ID5_MAPPING “Dropped in Migration”
  - 5) Validation Rules：Required fields、数据类型、业务规则（含 组合代码 条件默认、必填项、日期格式）
  - 6) Special Processing Notes：列出 edge cases、legacy quirks、已知问题（含手工映射覆盖、ID5 废弃）
  - 7) Parity Validation Checklist：写入可执行清单（行数、列名、数据值、日期格式、null 处理、company_id 一致性、ID5 禁用核查）
  - 8) Security / DB / Performance：目标表/Schema、主键/索引、NULL/类型要求、访问控制/审计要求、性能目标（行量、时延）、集成/部署路径
  - 9) Debugging & Exception Cases：常见异常示例（计划号缺失、机构名称缺匹配、组合代码异常、日期异常、ID5 禁用验证）、排查步骤与 expected vs actual
  - 10) Compatibility Notes：pandas/pydantic 版本注意事项；若无变更，注明 “no known breaking changes” 且引用 `docs/architecture/technology-stack.md`
  - Changelog：添加当前日期与作者
- 语言：正文使用 English；但可保留中文列名/示例；保持简洁 bullet，突出高信号提示
- 完成定义（DoD）：文档通过模板核对、AC 勾选、Parity checklist 可执行、路径与索引一致、评审人签字位预留
...
- [x] Task 1: Analyze Legacy AnnuityIncomeCleaner Code (AC: 1, 2, 3, 4, 5, 6)
  ...
  - [x] 1.4: Document conditional logic for 组合代码 default
  - [x] 1.5: **Document Manual Overrides:** List all hardcoded overrides in `COMPANY_BRANCH_MAPPING` (e.g., '内蒙': 'G31')
...
- [x] Task 3: Create Cleansing Rules Document (AC: 13, 14, 15, 16)
  ...
  - [x] 3.3: Complete Column Mappings table（执行序优先级、Transformation/Notes 填满）
  - [x] 3.4: Complete Cleansing Rules table（CR-001..，含优先级）
  - [x] 3.5: Complete Company ID Resolution Strategy section（标准链 + ID5 废弃说明）
  - [x] 3.6: Complete Validation Rules section（必填、类型、业务规则）
  - [x] 3.7: **Document Utility Logic:** Explicitly list `clean_company_name` regex patterns/rules
  - [x] 3.8: Include Parity Validation Checklist（可执行步骤，含 ID5 禁用核查）
...
- [x] Task 5: Validation (AC: 10, 11, 12, 14, 15)
  ...
  - [x] 5.3: Add Security/DB/Performance + Deployment/Integration notes + Debugging/Exception cases + Compatibility notes（与 Tech Stack 对齐）
...
**Tech Stack Guardrails（Architecture → Technology Stack）**
- Python 3.10+, pandas（locked），Pydantic 2.11.7+，pandera（schema），Dagster（orchestration），uv（pkg），Ruff 0.12.12+，mypy 1.17.1+（strict）；遵守 Clean Architecture import 规则（infra/domain 不依赖 io/orchestration）。
- 参照 `docs/architecture/infrastructure-layer.md`：CompanyIdResolver、CleansingRegistry、standard transforms/pipeline 作为迁移基线；不引入新依赖。
- 文件与配置遵循 6-file standard（面向后续 Story 5.5.2）。

**Dependencies / Story Ordering**
- Epic 5.5 stories：本故事先行；5.5.2（实现）、5.5.3（parity 验证）、5.5.4（多域集成）依赖此文档。保持与 Epic 5.5 概要一致（`docs/epics/epic-5.5-pipeline-architecture-validation.md`）。
...
### 收入明细 Sheet Fields (AnnuityIncome-specific)

| Field | Type | Notes |
|-------|------|-------|
| 月度 | date | Requires parse_to_standard_date() |
| 机构 | string | Renamed to 机构代码 |
| 机构名称 | string | Used for COMPANY_BRANCH_MAPPING lookup |
| 计划号 | string | Used for company_id resolution |
| 客户名称 | string | Normalized via clean_company_name() |
| 业务类型 | string | Maps to 产品线代码 |
| 计划类型 | string | Used for DEFAULT_PORTFOLIO_CODE_MAPPING |
| 组合代码 | string | Regex cleaned, conditional default |
| 收入金额 | numeric | Domain-specific field |
| 年金账户名 | string | **Derived:** Copy of 客户名称 before normalization |

### Cleansing Operations (Execution Order)

| # | Operation | Field | Logic | Rule Type |
|---|-----------|-------|-------|-----------|
| 1 | Column rename | 机构 → 机构代码 | `df.rename(columns={'机构': '机构代码'})` | mapping |
| 2 | Mapping | 机构代码 | `df['机构名称'].map(COMPANY_BRANCH_MAPPING)` | mapping |
| 3 | Date parse | 月度 | `parse_to_standard_date()` | date_parse |
| 4 | Default value | 机构代码 | Replace 'null' with 'G00', fillna('G00') | default_value |
| 5 | Regex replace | 组合代码 | `str.replace('^F', '', regex=True)` | regex_replace |
| 6 | Conditional default | 组合代码 | 职年受托/职年投资 → 'QTAN003', else DEFAULT_PORTFOLIO_CODE_MAPPING | conditional |
| 7 | Mapping | 产品线代码 | `df['业务类型'].map(BUSINESS_TYPE_CODE_MAPPING)` | mapping |
| 8 | Copy | 年金账户名 | `df['年金账户名'] = df['客户名称']` (before normalization) | copy |
| 9 | Normalize | 客户名称 | `clean_company_name()` | normalize |
| 10 | Company ID | company_id | `_update_company_id(plan_code_col='计划号', customer_name_col='客户名称')` | resolution |
| 11 | Fallback | company_id | `df['年金账户名'].map(COMPANY_ID5_MAPPING)` when company_id is empty | fallback |

### Mapping Tables Reference

| Mapping Table | Purpose | Source | Shared? |
|---------------|---------|--------|---------|
| `COMPANY_BRANCH_MAPPING` | 机构名称 → 机构代码 | `mappings.py:88-110` (DB + manual overrides) | Yes |
| `BUSINESS_TYPE_CODE_MAPPING` | 业务类型 → 产品线代码 | `mappings.py:11-23` (DB query) | Yes |
| `DEFAULT_PORTFOLIO_CODE_MAPPING` | 计划类型 → 默认组合代码 | `mappings.py:71` (static) | Yes |
| `COMPANY_ID5_MAPPING` | **DEPRECATED** (Legacy only) | `mappings.py:180-194` | **Dropped** |

### ⚠️ COMPANY_BRANCH_MAPPING Complete Values
...
### 🚨 DEPRECATION NOTICE: COMPANY_ID5_MAPPING
...
- `YYYY年MM月` → datetime (adds day 1)
- `YYYY年MM月DD日` → datetime
- `YYYYMMDD` → datetime
- `YYYYMM` → datetime (adds day 01)
- `YYYY-MM` → datetime (adds day -01)
- Other formats → `dateutil.parser.parse()`
...
2. Remove "及下属子企业"
3. Remove suffixes: `(团托)`, `-[A-Za-z]+`, `-\d+`, `-养老`, `-福利`
...
| 组合代码 Processing | Default only | Regex '^F' removal + conditional default |
| 年金账户名 | Exists in data | **Derived** from 客户名称 |

**Shared Operations (~80%):**
- 机构代码 mapping via COMPANY_BRANCH_MAPPING
...
- 产品线代码 mapping from 业务类型

**AnnuityIncome-Specific:**
- 组合代码 regex replacement (remove '^F' prefix)
- 组合代码 conditional default based on 业务类型
- 年金账户名 copy before normalization (preserved but not used for ID resolution)
...
- Access & audit: follow platform standard (RBAC, audit logs); note if customer_name/plan_code classified as sensitive—document masking needs if any.
...
### Parity Validation Checklist（执行版，写入文档 Section 7）
- 行/列匹配：行数一致；列名/重命名后等价；排除设计改进列（若有需说明）。
- 数据值匹配：字段值 100% 一致；日期格式标准化；null 处理与 legacy 一致（含 'null'→'G00'）。
- Company ID：标准链 `_update_company_id` 一致；明确 ID5 fallback 被禁用且不在新链路；对比 AnnuityPerformance 行为。
- 规则覆盖：11 个清洗操作全部映射到 Cleansing Rules 表；手工映射覆盖项完整；regex/conditional/default 顺序一致。
- 结果归档：验证脚本参考 `docs/runbooks/legacy-parity-validation.md`；输出到 `tests/fixtures/validation_results/`，报告保存并链接。
...
1. **Primary:** Real `收入明细` data from `reference/monthly/{YYYYMM}/收集数据/数据采集/*年金终稿*.xlsx`
...
**Validation:**
- Document completeness check against template（无占位符）
- Cross-reference with legacy code（11 operations 全覆盖，含 ID5 废弃说明）
- Verify output filename matches `index.md` link
- 执行 Parity Validation Checklist（至少 dry-run 清单，记录到文档）
- DoD：文档审阅（Link + 审阅人签字位预留）、路径/链接验证、版本/日期更新
- Security/DB/Performance：列出目标表/约束/性能目标；若未知，标注 TODO 以防遗漏
- Compatibility：注明 pandas/pydantic/uv/dagster 无已知 breaking changes；若有，写出警告及迁移指引
- Debugging/Exception cases：列出典型异常（缺 plan_code/机构名称不匹配/组合代码异常/日期 parse 失败/company_id 为空），提供排查步骤与 expected vs actual 示例
...
- ✅ Created comprehensive cleansing rules document at `docs/cleansing-rules/annuity-income.md`
- ✅ Documented all 11 cleansing operations (CR-001 to CR-011) with execution priority
- ✅ Documented COMPANY_ID5_MAPPING deprecation decision (lines 269-272 dropped)
- ✅ Documented `clean_company_name()` regex patterns in detail
- ✅ Identified gap: `annuity_performance/constants.py` missing 6 manual COMPANY_BRANCH_MAPPING overrides
- ✅ Added executable Parity Validation Checklist with ID5 fallback verification
- ✅ Added Security/DB/Performance, Debugging/Exception cases, Compatibility notes sections
