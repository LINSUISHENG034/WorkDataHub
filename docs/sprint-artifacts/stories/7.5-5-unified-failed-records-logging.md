...
- [ ] **Task 4: Update domain services** (AC: 3, 4)

  > ⚠️ **Deferred:** Domain service integration requires deeper Dagster op refactoring. Infrastructure layer is ready. See Tech Debt backlog for follow-up.

  - [ ] 4.1 In `domain/annuity_performance/service.py`: Add `failed_records` collection
  - [ ] 4.2 Apply same pattern to `domain/annuity_income/service.py`
  - [ ] 4.3 Add `_serialize_raw_data(row: dict) -> str` helper
  - [ ] 4.4 After validation passes, remove legacy `export_error_csv()` calls
...
- 20/20 tests passing ✅ (verified after code review fixes)
- Existing validation tests still passing ✅
