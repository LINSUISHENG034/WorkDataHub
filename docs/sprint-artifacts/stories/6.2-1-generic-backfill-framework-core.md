...
5. **Backfill Service API 契约（domain 层）**
   - 新建 `GenericBackfillService`（domain/reference_backfill/service.py）：`run(df: pd.DataFrame, configs: list[ForeignKeyConfig], conn, add_tracking_fields=True) -> BackfillResult`，内部调用 `_topological_sort`、`derive_candidates`、`backfill_table`。
...
- [ ] Task 1: 配置模型（AC: #1, #7）
  - [ ] 1.1 在 `domain/reference_backfill/models.py` 增加 BackfillColumnMapping / ForeignKeyConfig / DomainForeignKeysConfig（pydantic v2）。
  - [ ] 1.2 错误信息友好化，覆盖缺字段/类型/未知 depends_on。
  - [ ] 1.3 单测：配置校验成功/失败、未知依赖报错。

- [ ] Task 2: 通用回填服务（AC: #2, #4, #5, #9, #10, #11, #12, #13, #14, #15）
  - [ ] 2.1 `GenericBackfillService` 类实现 `_topological_sort`、`derive_candidates`（optional 列支持）、`backfill_table`（幂等+追踪字段）、`run`（返回 BackfillResult）。
  - [ ] 2.2 循环/未知依赖抛 ValueError；缺 `source_column` 记录 warning 并跳过。
  - [ ] 2.3 性能：批量插入策略（去重后批量写），记录 rows/sec。
  - [ ] 2.4 日志与错误：结构化记录 processing_order/每表插入/跳过原因。

- [ ] Task 3: 配置与示例（AC: #3, #8）
  - [ ] 3.1 在 `config/data_sources.yml` 为 `annuity_performance` 添加 `foreign_keys`（四个 FK，全字段示例，depends_on 正确）。
  - [ ] 3.2 说明 schema 版本兼容与默认行为（缺少 foreign_keys 时不执行回填）。

- [ ] Task 4: 集成与复用（AC: #6, #11, #15）
  - [ ] 4.1 orchestration ops 切换到 `GenericBackfillService`，保持依赖方向，保留 discover/read/loader 现有模式。
  - [ ] 4.2 废弃旧函数 `derive_plan_candidates`/`derive_portfolio_candidates` 的调用路径（保留临时 shim 或直接替换）。
  - [ ] 4.3 指定调用接口、参数与返回结构，增加日志/错误处理。

- [ ] Task 5: 数据库与安全（AC: #8, #11, #14）
  - [ ] 5.1 定义四张表的主键/必填/索引与追踪列写入规则（故事内 DDL 示例，代码中按列名写入）。
  - [ ] 5.2 确保 SQLAlchemy 参数化、权限最小化、日志不含 PII/凭据。

- [ ] Task 6: 测试与验证（AC: #16, #17）
  - [ ] 6.1 维护/扩展 `scripts/validation/verify_backfill_integrated.py`（如需更新）并确保 6/6 通过。
  - [ ] 6.2 单元/集成/性能测试新增：可选列、缺依赖、幂等、tracking 字段、ops 调用路径。
  - [ ] 6.3 记录性能数字、处理顺序、插入计数。
...
**Schema & Tracking Fields (示例 DDL)**

```sql
-- 追踪列由 Story 6.2.2 迁移提供；本故事写入这些列
CREATE TABLE "年金计划" (
  "年金计划号" VARCHAR PRIMARY KEY,
  "名称" VARCHAR,
...
-- 组合计划 / 产品线 / 组织架构 同理，主键列分别为 组合代码 / 产品线代码 / 组织代码
-- 插入 auto_derived 时覆盖 _source='auto_derived', _needs_review=true, _derived_from_domain=<domain>, _derived_at=now()
```
...
- orchestration/ops：用 settings.data_sources_config 读取 YAML → pydantic 校验 → 注入 GenericBackfillService。保持 “domain 不依赖 io/orchestration”；ops 负责 I/O、日志、错误冒泡（ValueError/自定义 DomainError）。
...
| `domain/reference_backfill/service.py` | **Major Refactor** | Replace with GenericBackfillService |
...
- Service class in `service.py` following existing pattern
...
- [Source: src/work_data_hub/domain/reference_backfill/service.py] - Current implementation to replace
