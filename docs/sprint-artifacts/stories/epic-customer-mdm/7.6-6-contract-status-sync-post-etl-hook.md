...
- [ ] [AI-Review][MEDIUM] Add unit tests for `contract_sync.py` covering sync idempotency, edge cases (empty source, FK violations) [contract_sync.py]
- [ ] [AI-Review][MEDIUM] Add unit tests for `hooks.py` covering hook registration/execution [hooks.py]
- [ ] [AI-Review][MEDIUM] Document SCD Type 2 v1 simplified implementation: current `ON CONFLICT DO NOTHING` does not track historical status changes. Clarify in specification if true SCD versioning (close old records) is expected. [customer-plan-contract-specification.md]
