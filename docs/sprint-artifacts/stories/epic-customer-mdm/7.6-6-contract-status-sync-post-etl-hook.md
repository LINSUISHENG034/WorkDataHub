...
- [ ] Task 1: Create Alembic migration for `customer_plan_contract` table (AC: 1)
  - [ ] Create migration file `io/schema/migrations/versions/008_create_customer_plan_contract.py`
  - [ ] Define table DDL with columns, constraints, indexes
  - [ ] Add `updated_at` trigger function
  - [ ] Create `config/customer_mdm.yaml` configuration file (see Configuration File section)
...
- [ ] Task 4: Add `--no-post-hooks` CLI flag (AC: 3)
  - [ ] Update `src/work_data_hub/cli/etl/main.py` with new flag (add around line 175, after `--skip-facts`):
    ```python
    parser.add_argument(
        "--no-post-hooks",
        action="store_true",
    ...
    ```
  - [ ] Pass flag through to executor via `args.no_post_hooks`
...
### Review Follow-ups (AI) — Added 2026-01-19

- [x] [AI-Review][MEDIUM] Add unit tests for `contract_sync.py` covering sync idempotency, edge cases (empty source, FK violations) [contract_sync.py] → **Resolved in Story 7.6-12** (unit tests added for has_status_changed, NULL handling)
- [ ] [AI-Review][MEDIUM] Add unit tests for `hooks.py` covering hook registration/execution [hooks.py]
- [x] [AI-Review][MEDIUM] Document SCD Type 2 v1 simplified implementation: current `ON CONFLICT DO NOTHING` does not track historical status changes. Clarify in specification if true SCD versioning (close old records) is expected. [customer-plan-contract-specification.md] → **Resolved in Story 7.6-12**
...
# after the execution summary is printed but before `return 0 if result.success else 1`:
...
config/
└── customer_mdm.yaml  # [NEW] Configuration for Customer MDM
```
...
- Alembic migrations follow `00X_xxx.py` naming (next available: **008**)

**Schema Context (CRITICAL):**
- `customer` schema already exists (created in Story 7.6-0) - **DO NOT** run `CREATE SCHEMA`
- `customer."年金客户"` table already exists and is the correct FK target (migrated from `mapping` schema)
- FK constraint `fk_contract_company` references `customer."年金客户"(company_id)` (NOT `mapping`)
- See `config/foreign_keys.yml` lines 160-164 for FK configuration
...
> [!IMPORTANT]
> **Create this configuration file in Task 1.** This is a required file, not optional.

> [!NOTE]
> For Story 7.6-6, only `status_year` is actively used to initialize placeholder values.
> `strategic_threshold` and `whitelist_top_n` are reserved for Story 7.6-9 (Index & Trigger Optimization)
> and can be left as placeholder values for now.
...
**Record Counts:**
- Initial population: 19,882 records
...
**Git Commits:**
- 3befd30 feat(config): add customer_mdm configuration file
- e3a5c32 feat(schema): create customer_plan_contract table with SCD Type 2 support
- b83d691 feat(customer): add contract sync service with idempotent upsert logic
- 6e248fd feat(etl): add post-ETL hook infrastructure for automatic data sync
- a4c116c feat(etl): register contract status sync hook for annuity_performance domain
- ea2e83a feat(etl): integrate post-ETL hook execution into domain pipeline
- 1ff6cff feat(cli): add --no-post-hooks flag to disable post-ETL hooks
- b0f0c59 feat(cli): add customer-mdm sync subcommand for manual contract sync
