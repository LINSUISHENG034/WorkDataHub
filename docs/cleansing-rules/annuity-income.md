...
> This document still contains migration-era analysis. The active implementation:
> 1. does **not** use legacy ID5 account-name fallback
> 2. **does** generate temporary `IN*` company IDs for unresolved cases in the
>    standard service path
> 3. may emit `unknown_names_csv` for manual review when export is enabled
...
- **Default Value:** Temporary `IN*` company_id generated by the shared resolver
- **Note:** The new implementation disables legacy ID5 fallback, but it does
  not leave unresolved company_id empty by default.
...
| company_id unresolved after standard matching | No mapping match | Temporary `IN*` ID | Temporary `IN*` ID | **Expected** - ID5 fallback disabled, temp ID fallback active |
