...
# 【可选】内网模式（启用后 EQC 请求将通过 PAC 代理，使用 PA_UM_ACCOUNT/PA_UM_PASSWORD 认证）
# WDH_INTRANET=true
# PA_UM_ACCOUNT=你的工号
# PA_UM_PASSWORD=你的密码
```
...
data/
└── real_data/
    └── {YYYYMM}/                          # 账期目录，如 202509
        └── 收集数据/
            ├── 数据采集/                    # annuity_performance / annuity_income
            │   └── *规模收入数据*.xlsx      # 包含「规模明细」和「收入明细」两个 Sheet
            └── 业务收集/                    # annual_award / annual_loss
                └── *台账登记*.xlsx          # 包含中标和流失相关 Sheet
```
...
**Bash / PowerShell：**
```bash
# 【试运行模式】Plan-Only：清洗逻辑运作，但最终拦截入库（排查清洗错误首选）
