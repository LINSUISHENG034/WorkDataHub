...
2. **AC-2**: View provides key metrics per business type per month:
   ...
   - `award_distinct_companies`: Distinct awarded company_ids
   - `loss_count`: Number of losses (流失客户数)
   - `loss_distinct_companies`: Distinct lost company_ids
...
- [x] Task 1: Create Alembic migration (AC: 4)
  - [x] Create `io/schema/migrations/versions/006_create_business_type_view.py`
  - [x] Define `upgrade()` with CREATE VIEW statement
  - [x] Define `downgrade()` with DROP VIEW statement
  - [x] Set proper revision chain (depends on 005)

- [x] Task 2: Design and implement the view SQL (AC: 1, 2, 3)
  - [x] Write CTE or UNION for combining 当年中标 and 当年流失 data
  - [x] Implement business type grouping (受托/投资)
  - [x] Add monthly aggregation with proper GROUP BY
  - [x] Handle NULL company_id exclusion for distinct counts

- [x] Task 3: Run migration and verify (AC: 5)
  - [x] Execute `alembic upgrade head` ✓ Migration applied
  - [x] Query view to verify correct aggregation ✓ 23 months of data verified
  - [x] Test with sample data from existing tables ✓ Verified via integration tests

- [x] Task 4: Documentation
  - [x] Update `docs/database-schema-panorama.md` with new view definition
  - [x] Add view to Sprint Change Proposal success criteria ✓ Verified 2026-01-30
...
- [Source: docs/project-planning-artifacts/sprint-change-proposal-2026-01-10.md#Story 7.6-3]
...
- Code Review: 2026-01-15 - 2 HIGH, 3 MEDIUM, 3 LOW issues fixed

### Completion Notes List

- Migration file created following 005 pattern
- View SQL uses CTE with FILTER for aggregation
- Documentation updated in database-schema-panorama.md
- Removed unused `_view_exists()` helper function

### File List

- `io/schema/migrations/versions/006_create_business_type_view.py` - NEW
- `docs/database-schema-panorama.md` - MODIFIED (Section 6.2 added)
