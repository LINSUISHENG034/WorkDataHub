...
- [x] Task 1: Implement Annual Cutover Function (AC: 1, 2, 3)
  - [x] Add `annual_cutover(year: int)` to `year_init.py`
  - [x] Create SQL file `sql/annual_cutover_close.sql`
  - [x] Create SQL file `sql/annual_cutover_insert.sql`
  - [x] Ensure transaction safety

- [x] Task 2: Recalculate Strategic Status (AC: 3)
  - [x] Query prior year December data
  - [x] Apply whitelist_top_n and strategic_threshold rules
  - [x] Set is_existing based on prior year presence

- [x] Task 3: Add CLI Command (AC: 5)
  - [x] Add `cutover` subcommand to `cli/customer_mdm/`
  - [x] Accept `--year` parameter (required)
  - [x] Add `--dry-run` mode for safety

- [x] Task 4: Add Unit Tests (AC: 4)
  - [x] `TestAnnualCutoverClosesAllRecords::test_annual_cutover_closes_all_records()`
  - [x] `TestAnnualCutoverCreatesNewRecords::test_annual_cutover_creates_new_records()`
  - [x] `TestAnnualCutoverIdempotency::test_annual_cutover_idempotency()`

- [x] Task 5: Integration Test (AC: 6)
  - [x] Run cutover for test year
  - [x] Verify January 1st records exist
...
- Implemented `annual_cutover(year, dry_run)` function in `year_init.py`
- Created SQL files for close and insert operations with proper JOIN to reference tables
- Added CLI `cutover` subcommand with `--year` (required), `--dry-run`, and `--yes` options
- Implemented idempotency via `ON CONFLICT DO NOTHING`
- Added year boundary validation (2020-2050)
- Added CLI confirmation prompt for non-dry-run execution
- All 19 unit tests pass, 1 integration test passes
...
| File | Change |
|------|--------|
| `src/work_data_hub/customer_mdm/year_init.py` | Added `annual_cutover()` function |
| `src/work_data_hub/customer_mdm/sql/annual_cutover_close.sql` | New - closes current records |
| `src/work_data_hub/customer_mdm/sql/annual_cutover_insert.sql` | New - inserts new year records |
| `src/work_data_hub/cli/customer_mdm/cutover.py` | New - CLI command |
| `src/work_data_hub/cli/__main__.py` | Added cutover subcommand routing |
| `tests/unit/customer_mdm/test_annual_cutover.py` | New - 19 unit tests |
| `tests/integration/customer_mdm/test_annual_cutover.py` | New - 1 integration test |

## Change Log

- 2026-02-06: Code review fixes applied:
  - Added 5 new behavior tests with mocking (H1/H2 fix)
  - Strengthened integration test assertions (H3 fix)
  - Added year boundary validation 2020-2050 (M2 fix)
  - Added CLI confirmation prompt with --yes bypass (M3 fix)
  - Enhanced completion logging with prior_year context (M4 fix)
- 2026-02-06: Story 7.6-14 implemented - Annual cutover function with CLI support
