...
def process_worksheet(df, annuity_code, portfolio_code):
    ...
    # 整理未入库年金计划号
    new_annuity_code = df[~df['计划代码'].isin(annuity_code['年金计划号'])].groupby(
        ['计划类型', '计划代码', '计划名称', '客户名称']
    )['业务类型'].apply(lambda x: '+'.join(sorted(set(x)))).reset_index()
    new_annuity_code.rename(columns={
        '计划代码': '年金计划号',
        '计划名称': '计划全称',
        '业务类型': '管理资格'
    }, inplace=True)
    ...
    # 整理未入库组合编号
    new_portfolio_code = df[~df['组合代码'].isin(portfolio_code['组合代码'])].groupby(
        ['计划代码', '组合代码', '组合名称', '组合类型']
    ).size().reset_index(name='Count').drop('Count', axis=1)
    new_portfolio_code.rename(columns={
        '计划代码': '年金计划号'
    }, inplace=True)
...
def prefix_processing(files):
    ...
    for file in tqdm(files, desc='Prefix Processing'):
        ...
        for sheet_name in ['规模明细', '收入明细']:
            try:
                ...
                logger.error(f'Error reading file {file} for sheet {sheet_name}', exc_info=e)
                continue
    ...
    with MySqlDBManager(database='mapping') as mysqldb:
        if new_annuity_codes:
            mysqldb.import_data('年金计划', pd.concat(new_annuity_codes).drop_duplicates(subset=['年金计划号'], keep='last'))
        if new_portfolio_codes:
            mysqldb.import_data('组合计划', pd.concat(new_portfolio_codes).drop_duplicates(subset=['组合代码'], keep='last'))
...
def import_company_id_mapping(start_time=None):
    ...
    try:
        ...
        if current_mapping.empty:
            ...
            current_mapping = current_mapping[['company_id', 'company_full_name', 'unite_code']]
            # 重命名列并添加类型标识
            current_mapping.rename(columns={'company_full_name': 'company_name'}, inplace=True)
        ...
        if former_mapping.empty:
            ...
            former_mapping.rename(columns={'company_former_name': 'company_name'}, inplace=True)
            former_mapping['type'] = 'former'

        # 合并 DataFrame
        company_id_mapping = pd.concat([former_mapping, current_mapping], ignore_index=True)
        ...
        # 删除重复数据
        company_id_mapping.drop_duplicates(subset=['company_id', 'company_name'], keep='last', inplace=True)
        ...
        with MySqlDBManager(database='enterprise') as mysqldb:
            mysqldb.import_data_with_ignore(
                'company_id_mapping',
                company_id_mapping
            )

        logger.info(f"Successfully imported {len(company_id_mapping)} records into MySQL.")
...
def sync_mongo_to_mysql(
    ...
    try:
        ...
        if data.empty:
            logger.info(f"No new data to sync from MongoDB '{mongo_table_name}' since {start_time}.")
            return
        ...
        with MySqlDBManager(database=mysql_db) as mysqldb:
            if mysql_table_name == 'business_info':
                if 'updateTime' in data.columns:
                    data['updateTime'] = pd.to_datetime(data['updateTime'], errors='coerce')
                else:
                ...
                mysqldb.import_data_with_update_on_conflict(mysql_table_name, data, 'updateTime')
            else:
                mysqldb.import_data_with_ignore(mysql_table_name, data)

        logger.info(f"Data synced from MongoDB '{mongo_table_name}' to MySQL '{mysql_table_name}' successfully.")

    except Exception as e:
        logger.error(f"Error syncing data from MongoDB '{mongo_table_name}' to MySQL '{mysql_table_name}': {e}")
        raise
...
def sync_enterprise_data(start_time: datetime = None):
    if not start_time:
        # 设置为当天午夜（UTC）
        start_time = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    sync_mongo_to_mysql('eqc_search_result', 'eqc_search_result', unique_fields=['key_word'], start_time=start_time)
    sync_mongo_to_mysql('base_info', 'base_info', start_time=start_time,
                        drop_columns=['highlightCode', 'highlight_content', 'highlight_code', 'nameHighlight'])
