...
def get_business_type_mapping(database_name='mapping', reverse=False):
    ...
    return {row[1]: row[0] for row in rows} if reverse else {row[0]: row[1] for row in rows}
...
def get_data_handler_mapping(database_name: str = 'config', update_frequency: str = 'daily'):
    update_frequency = update_frequency if update_frequency in ['daily', 'monthly'] else '%'
    with MySqlDBManager(database=database_name) as mysqldb:
        select_statement = f"""
...
# 默认组合代码设定
DEFAULT_PORTFOLIO_CODE_MAPPING = {'集合计划': 'QTAN001', '单一计划': 'QTAN002', '职业年金': 'QTAN003'}


def get_default_plan_code_mapping():
    with MySqlDBManager(database='mapping') as mysqldb:
        cursor = mysqldb.cursor
        cursor.execute(f''' SELECT 年金计划号, 组合代码 FROM 组合计划 WHERE 组合状态 != "虚拟"; ''')
        rows = cursor.fetchall()
