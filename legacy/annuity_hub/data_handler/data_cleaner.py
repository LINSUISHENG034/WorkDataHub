...
from data_handler.mappings import (
    ...
    COMPANY_ID1_MAPPING, COMPANY_ID2_MAPPING, COMPANY_ID3_MAPPING, COMPANY_ID4_MAPPING, COMPANY_ID5_MAPPING,
...
class AbstractCleaner(ABC):
    ...
    def clean(self) -> pd.DataFrame:
        try:
            ...
            logger.error(f'Error in class {class_name} cleaning {self.file_path}. Error: {e}')
    ...
    def _load_data(self, sheet=None) -> pd.DataFrame:
        try:
            if not(sheet is None) and self.file_path.endswith('.xlsx'):
                return pd.read_excel(self.file_path, sheet_name=sheet, header=0, dtype=str)
            elif self.file_path.endswith('.csv'):
                return pd.read_csv(self.file_path, sep='\t', header=0, encoding='UTF-16')
            elif self.file_path.endswith('.xls'):
                return pd.read_csv(self.file_path, sep='\t', header=0, encoding='UTF-16')
            elif self.file_path.endswith('.xlsx'):
                return pd.read_excel(pd.ExcelFile(self.file_path), sheet_name=0, header=0, dtype=str)
            else:
    ...
    def _get_data_from_file(self):
        if self.file_path.endswith('.csv'):
            try:
                local_data = pd.read_csv(self.file_path, delimiter=",", encoding="utf-8-sig", dtype=str)
                local_data.columns = local_data.columns.str.strip()
                return local_data
            except UnicodeDecodeError:
                try:
                    local_data = pd.read_csv(self.file_path, delimiter=",", encoding="cp936", dtype=str)
                    local_data.columns = local_data.columns.str.strip()
                    return local_data
                except UnicodeDecodeError:
    ...
    def _extract_department_code(self, df, column='G代码'):
        try:
            if column in df.columns:
                df['部门代码'] = np.where(df[column] == '总计', 'G05SZ99', df[column].str.extract(r'(G[\dA-Z]+)\)')[0])
                df['部门名称'] = np.where(df[column] == '总计', '', df[column].str.extract(r'\)(.+)')[0])
            return df
        except Exception as e:
    ...
    def _add_business_type_code(self, df):
        try:
            df['业务类型代码'] = df['业务类型'].str.lower().map(BUSINESS_TYPE_CODE_MAPPING)
    ...
    def _update_company_id(self, df, plan_code_col='计划代码', customer_name_col='客户名称', company_id_col='company_id'):
        """
    ...
    def _clean_method(self) -> pd.DataFrame:
        ...
        df['组合代码'] = df['组合代码'].mask(
            (df['组合代码'].isna() | (df['组合代码'] == '')), df.apply(
                lambda x: 'QTAN003' if x['业务类型'] in ['职年受托', '职年投资'] else DEFAULT_PORTFOLIO_CODE_MAPPING.get(x['计划类型']),
                axis=1)
        )
        # 匹配产品线代码
...
class GroupRetirementCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        df['机构代码'] = df['机构名称'].map(COMPANY_BRANCH_MAPPING)
        # 修正客户名称
        df['企业名称'] = df['企业名称'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
...
class HealthCoverageCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        df['考核业绩'] = df['最终业绩']
        # 安全删除列
        df.drop(columns=['所属渠道', '战区', '中心', '渠道', '战区', '缴费年', '缴费月', '新/续保', '折算后业绩', '最终业绩'], inplace=True, errors='ignore')
        # 修正客户名称
        df['客户名称'] = df['客户名称'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
        # 根据 '客户名称' 补充 'company_id'
        if 'company_id' not in df.columns:
...
class YLHealthCoverageCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        df.rename(columns={'机构': '机构名称'}, inplace=True)
        # 安全删除列
        df.drop(columns=['所属渠道', '战区', '中心', '渠道', '项目简称', '战区', '缴费年', '缴费月', '新/续保'], inplace=True, errors='ignore')
        # 修正客户名称
        df['客户名称'] = df['客户名称'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
        # 根据 '客户名称' 补充 'company_id'
        if 'company_id' not in df.columns:
...
class JKHealthCoverageCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        # 安全删除列
        del_columns = [col for col in df.columns if
                       '渠道' in col or col in ['拆分比例', '币种', '战区', '中心', '缴费年', '缴费月', '项目起保日期', 'G代码']]
        df.drop(columns=del_columns, inplace=True, errors='ignore')
        # 使用正则表达式和捕获组直接替换列名
        df.columns = [re.sub(r'(.+)[\(\（](.+)[\)\）]', r'\1_\2', col.replace('\n', '')) for col in df.columns]
        df.rename(columns={
            '缴费任务': '投保单状态'
        }, inplace=True)
        # 修正客户名称
        df['投保人'] = df['投保人'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
        # 根据 '客户名称' 补充 'company_id'
        if 'company_id' not in df.columns:
            df['company_id'] = np.nan
        mask = df['company_id'].isna() | (df['company_id'] == '')
...
class IFECCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        # 修正客户名称和company_id
        df['客户名称'] = df['客户名称'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
        df = self._update_company_id(df, plan_code_col='年金计划号', customer_name_col='客户名称')
...
class APMACleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        # 修正客户名称
        df['客户名称'] = df['客户名称'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
        df = self._update_company_id(df, plan_code_col='计划代码', customer_name_col='客户名称')
...
class TrusteeAwardCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        df['机构代码'] = df['机构'].map(COMPANY_BRANCH_MAPPING)
        # 修正客户名称
        df['客户全称'] = df['客户全称'].apply(lambda x: clean_company_name(x) if isinstance(x, str) else x)
        # 删除无效字段
        df.drop(columns=[
            'insert_sql', ''
        ], inplace=True, errors='ignore')
...
class HistoryFloatingFeesCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        if '组合代码' not in df.columns:
            ...
            df['组合代码'] = df['组合代码'].str.replace('^F', '', regex=True)
        # 向下填充空值
        df[['组合代码', '组合名称', '浮动金额']] = df[['组合代码', '组合名称', '浮动金额']].fillna(method='ffill')
        # 补充计划代码
        df['计划代码'] = df['组合代码'].map(DEFAULT_PLAN_CODE_MAPPING)

        # 只保留存在的列
        columns_needed = ["区域", "机构", "受托人", "组合代码", "组合名称", "浮动金额", "浮费年度", "已计提浮动金额", "计提月度"]
...
class AssetImpairmentCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        # 机构代码
        df['机构代码'] = df['分支机构'].str.replace('分公司汇总', '').map(COMPANY_BRANCH_MAPPING)
        # 修正日期
        df['月度'] = df['月度'].apply(parse_to_standard_date)
        # 产品线代码
...
class RevenueDetailsCleaner(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        dfs = []
        for sheet_name in ['公司利润数据']:  # 原读取表sheet_name'实际_2024', '实际_2023'
            ...
            df['月份'] = pd.to_datetime(df['月份'].str.replace(r'(\d+)月累计', r'\1', regex=True).apply(
                lambda x: f"{year}-{int(x):02d}-01"), format="%Y-%m-%d")
            # 字段映射
...
class AnnuityRateStatisticsData(AbstractCleaner):
    def _clean_method(self) -> pd.DataFrame:
        ...
        df['计划组合代码'] = df['计划组合代码'].mask(
            (df['计划组合代码'].isna() | (df['计划组合代码'] == '')) & (df['计划类型'] == '单一计划'), 'AN002')
        # 修正日期
        ...
        # 补充投资字段
        df['组合代码'] = df['组合代码'].mask(
            df['业务类型'] == '投资', df['计划组合代码'].str.replace('^F', '', regex=True))
        df['计划代码'] = df['计划代码'].mask(
...
if __name__ == '__main__':
    ...
    if selected_sub_process == 1:
        ...
        cleaner = AnnuityPerformanceCleaner(path=file_path, sheet_name='2312企年投资集合计划当月')
        data = cleaner.clean()
        # 写入数据到Excel
