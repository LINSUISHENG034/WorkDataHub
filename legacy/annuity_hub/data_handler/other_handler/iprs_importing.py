    ...
    with MySqlDBManager(database="iprs") as mysqldb:
        for file in files:
            ...
            conditions = [
                ...
                df["套餐名称"].eq("深圳重特大疾病保险")
                | df["业务大类"].eq(
                    "笼子外非大病"
                ),  # 深圳重疾险对应组合险代码P0556SZ99
                df["主险代码"].isin(
                    ["P0782", "P078201"]
                ),  # 个账产品六年期组合险代码P0782SZ99
                df["主险代码"].isin(
                    ["P0783", "P078301"]
                ),  # 个账产品一年期组合险代码P0783SZ99
                df["险种分类"].isin(["一年以上定期", "终身"])
                | df["业务大类"].eq("长险"),  # 长险业务对应组合险代码P0990SZ00
            ...
            # 向量化字符串操作，简单且更有效率
            df["投保人名称"] = (
                df["投保人名称"].str.replace("(", "（").str.replace(")", "）")
            )
            ...
            df = pd.concat([df[invalid_gp_mask], df[~invalid_gp_mask]]).drop_duplicates(
                subset="保单号"
            )

            mysqldb.import_data_with_update_on_conflict(
                table_name="承保清单_2024", data=df, update_field="承保日期"
            )
