...
class DataProcessor:
    def __init__(self, **kwargs):
        ...
        self.database_manager = kwargs.get(
            "database_manager", MySqlDBManager(database=self.target_database)
        )
        self.data = []
    ...
    def delete_existing_records(
        self, db: MySqlDBManager, data: pd.DataFrame, fields: str
    ):
        """
    ...
    def _import(self, db: MySqlDBManager):
        ...
        if not set(import_columns).issubset(set(database_columns)):
            info = {
                "table_name": self.target_table,
                "import_columns": sorted(import_columns),
                "database_columns": sorted(database_columns),
            }
            logger.error(info)
            raise ValueError(
                f"Import columns do not match database columns for table {self.target_table}"
            )
    ...
    def _handle_failed_import(data: pd.DataFrame, table_name: str):
        ...
        try:
            temp_db_manager.import_data(
                table_name=temp_table_name, data=data, delete_existing=False
            )
            logger.info(
                f"Failed data imported to temp database in table {temp_table_name}"
            )
