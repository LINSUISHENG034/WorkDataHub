...
class MySqlDBManager:
    ...
    def _insert_data(self, table_name: str, data: pd.DataFrame, batch_size: int, ignore_conflicts=False):
        try:
            # 分批插入数据
            for start_row in range(0, data.shape[0], batch_size):
                ...
                columns = ', '.join([f'`{col}`' for col in batch_data.columns])  # 字段名使用反引号包围
                ...
                # 批量执行插入
                self.cursor.executemany(insert_clause, [tuple(r[col] for col in batch_data.columns) for r in records])
    ...
    def import_data(self, table_name: str, data: pd.DataFrame, delete_existing: bool = False, batch_size: int = 1000):
        """
        直接插入数据，遇到主键冲突会抛出异常。
        ...
        try:
            ...
            self._insert_data(table_name, data, batch_size, ignore_conflicts=False)  # 不忽略主键冲突
    ...
    def import_data_with_ignore(self, table_name: str, data: pd.DataFrame, batch_size: int = 1000):
        """
        插入数据，跳过主键冲突
        参数:
        - table_name (str): 表名。
    ...
    def import_data_with_update_on_conflict(self, table_name: str, data: pd.DataFrame, update_field: str,
                                            batch_size: int = 1000):
        """
        ...
        try:
            ...
            self.conn.commit()  # 提交事务
            logger.info(f"Data imported to {table_name} with update on conflict based on {update_field}.")
        except Exception as e:
            self.conn.rollback()  # 发生错误时回滚
    ...
    def add_foreign_key(self, table_name: str):
        ...
        foreign_keys_mapping = {
            ...
            '年金计划号': {'fk_database': 'mapping', 'fk_table': '年金计划', 'alias': ['计划代码', '计划号']},
            '组合代码': {'fk_database': 'mapping', 'fk_table': '组合计划', 'alias': ['组合计划代码', '组合编号']},
        ...
        for index, mapping in foreign_keys_mapping.items():
            ...
            # 使用生成器表达式找到第一个匹配的列名
            column_name = next((name for name in potential_keys if name in columns), None)
            ...
            if column_name:
                ...
                if constraint_name in current_keys:
                    logger.info(f"Constraint '{constraint_name}' for column '{column_name}' already exists. Skipping.")
                    continue
                ...
                try:
                    ...
                    # 如果执行过程中出错，则回滚并记录错误
                    self.conn.rollback()
                    logger.error(f"Error adding foreign key to '{table_name}' for column '{column_name}': {e}")
                    ...
                    if self.column_exists(table_name, index_key):
                        ...
                        if not self.index_exists(table_name, index_name):
                            try:
                                ...
                                self.conn.rollback()
                                raise RuntimeError(f"Error altering table {table_name}: {e}")
                        else:
                            logger.info(f"Index {index_name} already exists on {table_name}.")
                    else:
                        logger.warning(f"Column {index_key} does not exist in table {table_name}.")
    ...
    def delete_rows_by_criteria(self, table_name: str, criteria: pd.DataFrame, fields: list, batch_size: int = 100):
        """
        ...
        for i in range(0, len(criteria), batch_size):
            ...
            for _, row in batch_criteria.iterrows():
                ...
                delete_statement = f'DELETE FROM {table_name} WHERE ' + ' AND '.join(conditions)
        ...
        for month in months:
            if re.match(r'\d{4}-\d{2}', month):
                try:
                    ...
                    total_deleted += self.cursor.rowcount  # Update the counter with the number of rows deleted
                except SQLAlchemyError as e:
    ...
    def delete_rows_by_values(self, table_name: str, values: list, col_name='保单号', batch_size=100):
        """
        ...
        for i in range(0, len(values), batch_size):
            ...
            delete_statement = f'DELETE FROM {table_name} WHERE {col_name} IN ({placeholders})'
    ...
    def get_columns_info_from_all_tables(self):
        ...
        for table_name in tqdm(table_names, desc="Fetching Columns"):
            ...
            df['database_name'] = self.database  # Use self.database_name instead of hard-coded value
            dfs.append(df)
    ...
    def convert_float_to_double(self):
        try:
            ...
            for table_name, column_name in columns_to_alter:
                alter_statement = f"ALTER TABLE {table_name} MODIFY {column_name} DOUBLE"
    ...
    def create_table(self, table_name, data: pd.DataFrame, primary_key: str = None):
        ...
        if primary_key:
            try:
                alter_sql = f'ALTER TABLE `{table_name}` ADD PRIMARY KEY (`{primary_key}`);'
                self.cursor.execute(alter_sql)
            except Exception as e:
                self.conn.rollback()
                raise RuntimeError(f"Error altering table {table_name} to add primary key: {e}")

    def summarize_customer_fields(self, target_table: str, target_database: str = 'temp'):
        """
        ...
        for table_name in tqdm(table_names, desc='Processing Tables'):
            if table_name in cname_dict:
                ...
                try:
                    ...
                    self.conn.rollback()
                    logger.error(f"Error inserting data from {table_name} using field {field}: {e}")
    ...
    def read_data_from_excel(file_path, sheet_name='sheet0'):
        ...
        if file_path.endswith('.csv'):
            try:
                local_data = pd.read_csv(file_path, delimiter=",", encoding="utf-8-sig", dtype='str')
                local_data = local_data.rename(columns=str.strip)
                df = local_data
            except UnicodeDecodeError:
                try:
                    local_data = pd.read_csv(file_path, delimiter=",", encoding="cp936", dtype='str')
                    local_data = local_data.rename(columns=str.strip)
                    df = local_data
                except UnicodeDecodeError:
            ...
            local_data = pd.read_excel(file_path, engine='openpyxl', sheet_name=sheet_name)
            df = local_data
        ...
        for table_name in tqdm(tables, desc='Adding company_id column'):
            if table_name in cname_dict:
                ...
                if not self.column_exists(table_name, 'company_id'):
                    try:
                        # 添加 company_id 列和索引
                        self.cursor.execute(f'ALTER TABLE `{table_name}` ADD COLUMN `company_id` VARCHAR(50) NULL;')
                        self.cursor.execute(f'CREATE INDEX `KY_客户号` ON `{table_name}` (`company_id`);')
                        self.conn.commit()
                    except Exception as e:
                        self.conn.rollback()
                        logger.error(f"在表 {table_name} 中添加 company_id 列时出错: {e}")
    ...
    # 数据库字段转换
        ...
        for db in ['enterprise']:
            with MySqlDBManager(database=db) as mysqldb:
                ...
                data = mysqldb.get_full_table(sql_statement=f'SELECT * FROM `{old_mysql_table_name}`;')
                # 数据清洗
                from common_utils.common_utils import clean_company_name
