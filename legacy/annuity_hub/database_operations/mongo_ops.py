...
class MongoDBManager:
    def __init__(
        self, uri="mongodb://localhost:27017/", database_name="enterprise_data"
    ):
        self.uri = uri
    ...
    def _handle_failed_insert(self, data, collection_name):
        ...
        try:
            ...
            logger.info(
                f"Failed data inserted into temp collection {temp_collection_name}"
            )
        except PyMongoError as e:
            logger.error(
                f"Error inserting data into temp collection {temp_collection_name}: {e}"
            )
    ...
    def get_collection_fields(self, collection_name: str):
        ...
        try:
            ...
            if sample_record:
                ...
                logger.info(
                    f"Retrieved fields for collection {collection_name}: {fields}"
                )
                return fields
            else:
    ...
    def get_latest_unique_data(
        self, collection_name, unique_fields, time_field=None, start_time=None
    ):
        collection = self.db[collection_name]
        ...
        try:
            ...
            if start_time is not None:
                ...
                if time_field == "_id":
                    ...
                    pipeline.append({"$match": {time_field: {"$gte": start_object_id}}})
                else:
                    # 其他时间字段，直接使用 datetime 比较
                    pipeline.append({"$match": {time_field: {"$gte": start_time}}})
            ...
            logger.info(
                f"Retrieved {len(unique_data)} unique documents from {collection_name}"
            )
            return df
        except PyMongoError as e:
    ...
    def merge_fields_to_dataframe(
        ...
        try:
            ...
            if not documents:
                logger.info(
                    f"No documents found in {collection_name} after {start_time}"
                )
            ...
            if not expand_array:
                ...
                for doc in documents:
                    array_lengths = [
                        len(doc.get(field)) if isinstance(doc.get(field), list) else 1
                        for field in fields
                    ]
                    ...
                    for i in range(max_len):
                        ...
                        for field in fields:
                            if isinstance(doc.get(field), list):
                                expanded_doc[field] = (
                                    doc[field][i] if i < len(doc[field]) else None
                                )
                            else:
            ...
            logger.info(
                f"Converted documents from {collection_name} to DataFrame with shape {df.shape}"
            )
            return df
        except PyMongoError as e:
            logger.error(
                f"Error merging fields from {collection_name} to DataFrame: {e}"
            )
            raise
