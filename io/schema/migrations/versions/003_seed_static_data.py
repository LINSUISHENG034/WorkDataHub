...
- Enterprise schema: company_types_classification (104 rows), industrial_classification (1,183 rows)
- Mapping schema: 产品线 (12 rows), 组织架构 (38 rows), 计划层规模 (7 rows), 产品明细 (18 rows), 利润指标 (12 rows)

All INSERT statements use ON CONFLICT DO NOTHING for idempotency.

CRITICAL: All seed data comes from legacy database (config/seeds/*.csv), version-controlled.
...
def _load_csv_seed_data(conn, csv_filename: str, table_name: str, schema: str) -> int:
    ...
    # Use safe parameter names (param_0, param_1, etc.) to handle special chars in column names
    ...
    for row in rows:
        ...
        # Skip empty rows (all values are empty strings after normalization)
