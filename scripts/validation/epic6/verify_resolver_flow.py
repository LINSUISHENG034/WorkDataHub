...
def test_pipeline_flow_verification():
    ...
    df = pd.DataFrame({
        "客户名称": [raw_name],
        "company_id": [company_id], # Already resolved via some other method (e.g. Account Number)
        "年金账户号": ["ACC123"]
    })
    ...
    if call_args:
        ...
        name_mapping = next((m for m in inserted_mappings if m['match_type'] == 'name'), None)
    ...
    if lookup_call_args:
        ...
        if expected_normalized in looked_up_keys:
        ...
        elif raw_name in looked_up_keys: # Note: lookup logic usually doesn't even strip!
            ...
            print(f"❓ FINDING: Lookup uses unexpected keys.")
    else:
