...
print("=" * 70)
print("检查2025年10月规模明细客户是否全部被客户年金计划覆盖")
print("=" * 70)
...
print(f"2. 客户年金计划 当前有效记录的唯一客户数: {cpc_customers}")
...
if missing_customers > 0:
    ...
    cur.execute("""
        ...
        SELECT 
            CASE 
                WHEN yj.company_id IS NULL THEN 'FK: 客户明细表无此客户'
                ELSE '其他原因'
            END as reason,
            COUNT(*) as cnt
        FROM missing m
        LEFT JOIN customer."客户明细" yj ON m.company_id = yj.company_id
        GROUP BY 1
        ORDER BY 2 DESC
    """)
    print("\n4. 未覆盖客户原因分析:")
    ...
    cur.execute("""
        ...
        SELECT 
            m.company_id,
            yj.客户名称,
            CASE WHEN yj.company_id IS NULL THEN '不在客户明细表' ELSE '在客户明细表' END
        FROM missing m
        LEFT JOIN customer."客户明细" yj ON m.company_id = yj.company_id
        LIMIT 10
    """)
