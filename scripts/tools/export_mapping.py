...
class MappingExtractor:
    ...
    def extract_mapping_from_database(self, plan: ExtractionPlan) -> MappingExport:
        ...
        if not plan.table_name:
            raise ValueError(
                f"Table name required for database extraction of {plan.mapping_name}"
            )
        if not plan.sql_query:
            raise ValueError(
                f"SQL query required for database extraction of {plan.mapping_name}"
            )

        logger.info(
            f"Extracting {plan.mapping_name} from database table {plan.table_name}"
        )
        ...
        with psycopg2.connect(self.dsn) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if not self.check_table_exists(cursor, plan.table_name):
                    error_msg = (
                        f"Table {plan.table_name} not found for {plan.mapping_name}"
                    )
    ...
    def generate_missing_entries_report(self):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            if not self.missing_entries:
                ...
                f.write("\n## Recommendations\n\n")
                f.write(
                    "- Verify that all referenced tables have been migrated to PostgreSQL\n"
                )
                f.write(
    ...
    def list_available_mappings(self):
        ...
        for i, (name, plan) in enumerate(self.extraction_plans.items(), 1):
            print(
                f"  {i:2d}. {name:<30} ({plan.source_type:<10}) -> {plan.output_file}"
            )
...
def main():
    ...
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available mappings"
    )
    parser.add_argument(
        "--output", "-o", help="Custom output file (only with --mapping)"
    )
