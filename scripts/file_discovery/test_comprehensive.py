...
for test in test_cases:
    ...
    try:
        ...
        if match_result.match_count > 1:
            ...
            results.append({'month': month, 'status': 'AMBIGUOUS', 'message': result, 'files': match_result.match_count})
        ...
        if 'Ambiguous' in error_msg or 'ambiguous' in error_msg.lower():
            result = f'✅ CORRECTLY ERRORED: Ambiguity detected (expected)'
            results.append({'month': month, 'status': 'AMBIGUOUS_ERROR', 'message': error_msg})
            ambiguity_detected += 1
        else:
            result = f'❌ UNEXPECTED ERROR: {error_type}'
            results.append({'month': month, 'status': 'ERROR', 'error_type': error_type, 'message': error_msg})
...
if success_count + ambiguity_detected == len(test_cases):
    ...
    print(f'   Success rate: {(success_count + ambiguity_detected) / len(test_cases) * 100:.0f}%')
