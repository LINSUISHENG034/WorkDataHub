...
for path in test_paths_202311:
    ...
    try:
        ...
        print(f'    ✅ SUCCESS: {version_result.version} | {excel_result.row_count:,} rows')
...
try:
    version_result = scanner.detect_version(base_path_202510, patterns, 'highest_number')
    ...
    if match_result.match_count > 1:
        ...
        print(f'    ❌ ISSUE: System should raise error but returned: {match_result.matched_file.name}')
    else:
        excel_result = reader.read_sheet(match_result.matched_file, '规模明细', normalize_columns=True)
...
if not base_path_202502.exists():
    ...
    try:
        version_result = scanner.detect_version(base_path_202502, patterns, 'highest_number')
