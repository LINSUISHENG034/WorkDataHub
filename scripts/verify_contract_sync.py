"""Verify 客户年金计划 sync logic with October 2025 data.

验证脚本：检查从 business.规模明细 到 customer."客户年金计划" 的同步逻辑
"""
...
def main():
    ...
    print("=" * 60)
    print("验证 客户年金计划 更新逻辑 - 2025年10月数据")
    print("=" * 60)
    ...
    # 查询4: 验证同步完整性 - 2025年10月的唯一业务键是否都在 客户年金计划 表中
    cur.execute("""
        WITH oct_biz AS (
            SELECT DISTINCT 
                company_id, 
    ...
    print("\n4. 同步完整性检查:")
    print(f"   - 2025年10月业务键中未同步到 客户年金计划 表的记录数: {missing_count}")
    if missing_count == 0:
    ...
    for row in cur.fetchall():
        print(
            f"   - {row[7] or 'N/A'}: status={row[3]}, strategic={row[4]}, existing={row[5]}"
        )
