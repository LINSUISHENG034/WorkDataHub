...
This module is imported by tests via:
    - tests add `scripts/` to sys.path, then import `migrations.*`

It intentionally provides a small, testable API surface:
  - LegacyMigrationConfig
  - MigrationReport / FullMigrationReport
  - migrate_company_id_mapping / migrate_eqc_search_result
  - rollback_migration
"""
...
class FullMigrationReport:
    ...
    def add_report(self, report: MigrationReport) -> None:
        ...
        self.total_errors += report.errors


def _estimate_conflicts(_connection: Any, _report: MigrationReport) -> int:
    """
    Estimate conflicts during batch insert.

    Tests patch this function to return a deterministic value.
    """
...
def migrate_company_id_mapping(
    ...
    def to_record(row: Dict[str, Any]) -> Optional[EnrichmentIndexRecord]:
        ...
        record_type = row.get("type") or "former"
        ...
        confidence = (
            ...
            if str(record_type).lower() == "current"
    ...
    # Support MagicMock connections in unit tests (they patch _fetch_* anyway).
    if isinstance(connection, Connection):
        rows_iter = _fetch_company_id_mapping(connection, config.company_id_mapping_table)
    else:
        rows_iter = _fetch_company_id_mapping(connection, config.company_id_mapping_table)  # type: ignore[arg-type]
...
def migrate_eqc_search_result(
    ...
    def to_record(row: Dict[str, Any]) -> Optional[EnrichmentIndexRecord]:
        ...
        normalized = normalize_for_temp_id(key_word)
        if not normalized:
            report.add_skipped("empty_after_normalization")
            return None
        if not company_id:
    ...
    if isinstance(connection, Connection):
        rows_iter = _fetch_eqc_search_result(connection, config.eqc_search_result_table)
    else:
        rows_iter = _fetch_eqc_search_result(connection, config.eqc_search_result_table)  # type: ignore[arg-type]
...
def rollback_migration(connection: Connection, *, force: bool = False) -> int:
    """
    Delete all records inserted by legacy migration.

    The integration test calls this with force=True.
    """
    if not force:
