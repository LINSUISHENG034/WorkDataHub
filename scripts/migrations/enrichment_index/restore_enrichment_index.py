...
def run_migration_script(script_name: str, dry_run: bool = False) -> int:
    ...
    result = subprocess.run(cmd, env=env, cwd=str(Path(__file__).parent.parent.parent.parent))
...
def main() -> int:
    ...
    migrations = [
        ("migrate_customer_name_mapping.py", "customer_name from company_id_mapping + eqc_search_result"),
        ("migrate_plan_mapping.py", "plan_code from mapping.年金计划"),
        ("migrate_account_number_mapping.py", "account_number from enterprise.annuity_account_mapping"),
