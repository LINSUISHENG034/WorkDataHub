...
from sqlglot_parser import SQLGlotParser, find_sql_file_for_table

# 配置日志
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.StreamHandler(), logging.FileHandler("sync_schema.log")],
)
...
class SchemaSynchronizer:
    ...
    def sync_table(self, schema: str, table_name: str, dry_run: bool = False) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"解析完成: {len(mysql_table_def.indexes)} 个索引, {len(mysql_table_def.foreign_keys)} 个外键"
            )
            ...
            ddl_statements = ddl_generator.generate_batch_ddl(
                mysql_table_def.indexes, mysql_table_def.foreign_keys
            )
            ...
            if dependency_report["can_create_foreign_keys"]:
                ...
                for fk in mysql_table_def.foreign_keys:
                    ref_table_info = dependency_report["foreign_key_dependencies"].get(
                        fk.ref_table, {}
                    )
                    if not ref_table_info.get("exists"):
                        report["skipped_items"].append(
                            f"外键 {fk.name} - 引用表 {fk.ref_table} 不存在"
                        )
            ...
            if not dry_run:
                execution_report = self._execute_ddl(
                    schema, table_name, indexes_to_create, foreign_keys_to_create
                )
            ...
            report["success"] = True
            logger.info(
                f"同步完成: {len(indexes_to_create)} 个索引, {len(foreign_keys_to_create)} 个外键"
            )
    ...
    def _execute_ddl(
        ...
        execution_report = {"executed_statements": [], "execution_errors": []}
...
def print_report(report: Dict):
    ...
    if report["mysql_table_def"]:
        ...
        print(
            f"发现: {len(mysql_def.indexes)} 个索引, {len(mysql_def.foreign_keys)} 个外键"
        )
...
示例:
  ...
  %(prog)s --table business.规模明细 --verbose
    ...
    parser.add_argument(
        "--table", required=True, help="目标PostgreSQL表名 (格式: schema.table_name)"
    )
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="只显示要执行的SQL，不实际执行"
    )

    parser.add_argument("--verbose", action="store_true", help="显示详细日志")

    parser.add_argument("--force", action="store_true", help="强制执行，忽略某些错误")
    ...
    # 获取数据库连接字符串
    db_connection = (
        os.getenv("WDH_DATABASE__URI")
        or os.getenv("DATABASE_URL")
        or "postgresql://postgres:Post.169828@localhost:5432/postgres"
    )
