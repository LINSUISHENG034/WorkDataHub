...
class PgloaderWrapper:
    ...
    def _check_pgloader_installation(self) -> None:
        ...
        try:
            result = subprocess.run(
                ["pgloader", "--version"], capture_output=True, text=True, check=True
            )
    ...
    def migrate_with_tables(
        ...
        config_file = self.create_config_file(
            source_uri=source_uri, target_uri=target_uri, tables=tables, **options or {}
        )
...
示例:
  ...
  %(prog)s --config migration.load --dry-run
    ...
    parser.add_argument("--source", help="Source MySQL connection URI")

    parser.add_argument("--target", help="Target PostgreSQL connection URI")

    parser.add_argument("--config", help="pgloader configuration file path")

    parser.add_argument("--tables", help="Comma-separated list of tables to migrate")

    parser.add_argument(
        "--schema-only", action="store_true", help="Migrate schema only, no data"
    )

    parser.add_argument(
        "--no-foreign-keys", action="store_true", help="Skip foreign key constraints"
    )
    ...
    if args.config:
        ...
        success = wrapper.migrate_databases(
            source_uri="", target_uri="", config_file=args.config, options=options
        )
    # 使用命令行参数
    elif args.tables:
        # 迁移指定表
        ...
        # 迁移整个数据库
        success = wrapper.migrate_databases(
            source_uri=args.source, target_uri=args.target, options=options
        )
