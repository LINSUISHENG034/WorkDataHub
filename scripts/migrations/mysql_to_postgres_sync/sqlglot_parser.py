...
class MySQLIndex:
    ...
    def __init__(self, name: str, columns: List[str], index_type: str = "INDEX",
                 unique: bool = False, where: Optional[str] = None):
        self.name = name
...
class MySQLForeignKey:
    ...
    def __init__(self, name: str, columns: List[str], ref_table: str,
                 ref_columns: List[str], on_delete: str = "RESTRICT",
                 on_update: str = "RESTRICT", match: Optional[str] = None):
        self.name = name
        self.columns = columns
    ...
    def __repr__(self):
        return (f"MySQLForeignKey(name='{self.name}', columns={self.columns}, "
...
class MySQLTableDefinition:
    ...
    def __repr__(self):
        return (f"MySQLTableDefinition(database='{self.database}', table='{self.table_name}', "
...
class SQLGlotParser:
    ...
    def _extract_table_info(self, create_stmt: exp.Create, database: str = "unknown") -> MySQLTableDefinition:
        """
        ...
        if isinstance(create_stmt.this, exp.Schema):
            # Schema对象的this属性包含表名
            if hasattr(create_stmt.this, 'this') and hasattr(create_stmt.this.this, 'name'):
        ...
        for constraint in schema_expressions:
            if isinstance(constraint, exp.PrimaryKey):
                ...
                if hasattr(constraint.this, 'this'):
                    ...
                    index_name = constraint.this.this.name if hasattr(constraint.this.this, 'name') else f"uk_{table_name}"
                    ...
                    index_name = constraint.this.name if hasattr(constraint.this, 'name') else f"uk_{table_name}"
                    columns = []
                ...
                if isinstance(constraint.this, exp.Identifier):
                    ...
                    index_name = constraint.this.name if hasattr(constraint.this, 'name') else f"idx_{table_name}"

                # 提取列名 - 从 constraint.args['expressions'] 或 constraint.expressions
                columns = []
                expressions = constraint.args.get('expressions') or constraint.expressions or []
                ...
                if constraint_expressions and isinstance(constraint_expressions[0], exp.ForeignKey):
                    fk_expr = constraint_expressions[0]
                    constraint_name = constraint.this.name if hasattr(constraint.this, 'name') else f"fk_{table_name}"
                    ...
                    if reference:
                        ...
                        if isinstance(ref_schema, exp.Schema):
                            ref_table = ref_schema.this.name if hasattr(ref_schema.this, 'name') else str(ref_schema.this)
                        else:
                            ref_table = ref_schema.name if hasattr(ref_schema, 'name') else str(ref_schema)
    ...
    def extract_foreign_keys_from_table(self, sql_content: str) -> List[MySQLForeignKey]:
        """
        从SQL内容中提取所有外键定义
...
class PostgreSQLDDLGenerator:
    ...
    def generate_index_ddl(self, index: MySQLIndex) -> Tuple[str, str]:
        ...
        if index.index_type == "PRIMARY":
            ...
            return (index.name, f"-- Primary key should be defined in CREATE TABLE")
    ...
    def generate_foreign_key_ddl(self, fk: MySQLForeignKey) -> Tuple[str, str]:
        ...
        # 构建DDL
        ddl = (f'ALTER TABLE "{self.schema}"."{self.table_name}" '
    ...
    def generate_batch_ddl(self,
                          indexes: List[MySQLIndex],
                          foreign_keys: List[MySQLForeignKey]) -> List[Tuple[str, str, str]]:
        """
        批量生成DDL语句
...
def find_sql_file_for_table(sql_dir: Path, schema: str, table_name: str) -> Optional[Path]:
    """
...
def main():
    ...
    if table_def:
        ...
        ddl_statements = ddl_gen.generate_batch_ddl(table_def.indexes, table_def.foreign_keys)
