"""
Generate annuity_performance seed data from Legacy MySQL for New Pipeline.

================================================================================
BACKGROUND & PURPOSE
================================================================================

This script generates seed data for the annuity_performance domain by:
1. Exporting raw data from Legacy MySQL (business.规模明细)
...
Step 0: Clear Legacy Custom Cleaned Values
-------------------------------------------
Problem: Values with 年金账户号 starting with "GM" are Legacy custom cleaned data.
Solution: Clear these values (set to NULL).
Rationale:
- This Legacy custom cleaning step has been removed in New Pipeline
- These values should not exist in seed data
- New Pipeline will re-determine the correct value
...
Step 5: Rename 年金账户号 to 集团企业客户号
---------------------------------------------
Problem: 年金账户号 field contains enterprise customer number data.
Solution: Rename to match New Pipeline's expected schema.
Rationale:
- New Pipeline expects 集团企业客户号 as source field
- New Pipeline will derive 年金账户号 = 集团企业客户号.str.lstrip("C")
- This renaming aligns with the data flow: source → derived

Step 6: Reset G00 Institution Codes
------------------------------------
...
---------------------------------
Problem: Column names have inconsistent formats.
Solution: 流失(含待遇支付) → 流失_含待遇支付.
Rationale:
- New Pipeline schema uses underscore format
- Parentheses can cause issues in some systems
...
Full Pipeline (Export + Clean):
    PYTHONPATH=src uv run python scripts/seed_data/generate_annuity_performance_seed.py

Export Only:
    PYTHONPATH=src uv run python scripts/seed_data/generate_annuity_performance_seed.py --export-only

Clean Only:
    PYTHONPATH=src uv run python scripts/seed_data/generate_annuity_performance_seed.py --clean-only --input data/seed_data/annuity_performance_raw.csv

With Limit (for testing):
    PYTHONPATH=src uv run python scripts/seed_data/generate_annuity_performance_seed.py --limit 10000

Custom Output Path:
    PYTHONPATH=src uv run python scripts/seed_data/generate_annuity_performance_seed.py -o data/my_output.csv
...
def export_data(
    ...
    """
    Export annuity_performance data from Legacy MySQL to CSV.
    ...
    try:
        with engine.connect() as conn:
            ...
            with open(metadata_path, "w", encoding="utf-8") as f:
                f.write("# Annuity Performance Export Metadata\n")
...
def clean_data(
    ...
    """
    Clean exported annuity_performance data for New Pipeline processing.
    ...
    # ========================================================================
    # Step 2.0: Clear Legacy Custom Cleaned Values (GM prefix)
    # ========================================================================
    print("\n🗑️  Step 2.0: Clearing Legacy custom cleaned values...")

    if "年金账户号" in df.columns:
        # Values with 年金账户号 starting with "GM" are Legacy custom cleaned data
        # This cleaning step has been removed in New Pipeline, clear these values
        gm_mask = df["年金账户号"].astype(str).str.startswith("GM")
        gm_count = gm_mask.sum()
        df.loc[gm_mask, "年金账户号"] = None
        print(f"   ✓ Cleared: {gm_count:,} values with 年金账户号 starting with 'GM'")
    else:
        print("   ⚠️ Warning: '年金账户号' column not found")
    ...
    # ========================================================================
    # Step 2.4: Rename 年金账户号 to 集团企业客户号
    # ========================================================================
    print("\n🔄 Step 2.4: Renaming 年金账户号 → 集团企业客户号...")

    if "年金账户号" in df.columns:
        df = df.rename(columns={"年金账户号": "集团企业客户号"})

        sample_enterprise_ids = df["集团企业客户号"].dropna().head(3).tolist()
        print("   ✓ Renamed to 集团企业客户号")
        print(f"   📋 Sample values: {sample_enterprise_ids}")
        print(
            "   📝 New Pipeline will derive: 年金账户号 = 集团企业客户号.str.lstrip('C')"
        )
    else:
    ...
    # ========================================================================
    # Step 2.5: Reset G00 Institution Codes to NULL
    # ========================================================================
    print("\n🔄 Step 2.5: Resetting 'G00' institution codes to NULL...")
    ...
    # ========================================================================
    print("\n🔄 Step 2.6: Standardizing column names...")

    if "流失(含待遇支付)" in df.columns:
        df = df.rename(columns={"流失(含待遇支付)": "流失_含待遇支付"})
        print("   ✓ Renamed: 流失(含待遇支付) → 流失_含待遇支付")
    else:
        print("   ℹ️  Column name already standardized or not found")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate annuity_performance seed data from Legacy MySQL",
...
Examples:
  ...
  %(prog)s -o data/my_output.csv
    ...
    default_filename = f"annuity_performance_{timestamp}.csv"
    ...
    if args.output:
        ...
        cleaned_output = DEFAULT_OUTPUT_DIR / default_filename
        raw_output = DEFAULT_OUTPUT_DIR / f"annuity_performance_{timestamp}_raw.csv"
    ...
    print(f"\n{'=' * 70}")
    print("ANNUITY PERFORMANCE SEED DATA GENERATION")
    print(f"{'=' * 70}")
