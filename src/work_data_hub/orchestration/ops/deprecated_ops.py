...
def process_sandbox_trustee_performance_op(
    ...
    """DEPRECATED: Use process_domain_op_v2 with domain='sandbox_trustee_performance'."""
    ...
    try:
        ...
        context.log.info(
            ...
            file_path, len(excel_rows), len(result_dicts),
        )
        return result_dicts
...
def process_annuity_income_op(
    ...
    try:
        ...
        context.log.info(
            ...
            mode_text, file_path, len(excel_rows), len(result_dicts),
        )
        return result_dicts
    except Exception as e:
        context.log.error(f"Domain processing failed: {e}")
...
def process_annual_award_op(
    ...
    try:
        ...
        context.log.warning(
            "Failed to initialize CompanyMappingRepository: %s", str(e)
        )
    ...
    try:
        ...
        context.log.info(
            ...
            mode_text, file_path, len(excel_rows), len(result_dicts), failed_count,
        )
        return result_dicts
    except Exception as e:
        context.log.error(f"Domain processing failed: {e}")
    ...
    try:
        ...
        if use_enrichment:
            ...
            enrichment_service = CompanyEnrichmentService(
                loader=loader, queue=queue, eqc_client=eqc_client,
                sync_lookup_budget=config.enrichment_sync_budget,
                observer=observer, enrich_enabled=settings.enrich_enabled,
            )
