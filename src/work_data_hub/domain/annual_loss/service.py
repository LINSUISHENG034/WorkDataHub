...
from .models import AnnualLossOut, AnnualLossProcessingResult
    ...
    *,
    ...
    Parameters:
        ...
        source_type: Which source sheet(s) to process:
            - "trustee": Only 企年受托流失(解约) sheet
            - "investee": Only 企年投资流失(解约) sheet
            - "both": Process both sheets (default)
        domain: Domain identifier (default: "annual_loss")
    ...
    all_records: List[AnnualLossOut] = []
    total_failed = 0
    ...
    logger.bind(domain=domain, step="pipeline_completed").info(
        "annual_loss.pipeline.completed",
        month=normalized_month,
        source_type=source_type,
...
def _process_source_sheet(
    ...
    # Use correct product line names per mapping
    ...
    try:
        ...
        return AnnualLossProcessingResult(
            records=[],
    ...
    context = PipelineContext(
        ...
        execution_id=f"annual_loss-{sheet_type}-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
        timestamp=datetime.now(timezone.utc),
        config={"domain": domain, "sheet_type": sheet_type},
        domain=domain,
        run_id=f"annual_loss-{sheet_type}-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
