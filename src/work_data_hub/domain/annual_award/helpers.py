...
from work_data_hub.domain.annual_award.models import AnnualAwardOut
...
def run_discovery(
    *, file_discovery: FileDiscoveryProtocol, domain: str, month: str
    ...
    try:
        ...
        logger.error("annual_award.discovery.failed", extra=error_ctx.to_log_dict())
        raise
...
def rename_customer_name_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This implements requirement #3 from the proposal:
...
def drop_excluded_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Drop excluded columns: 区域, 年金中心, 上报人.

    This implements requirement #1 from the proposal.
    """
    columns_to_drop = ["区域", "年金中心", "上报人"]
...
def conditional_update_plan_and_company(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This implements requirement #2 from the proposal:
...
def convert_dataframe_to_models(
    df: pd.DataFrame,
) -> Tuple[List[AnnualAwardOut], int]:
    """Convert DataFrame rows to AnnualAwardOut models.

    Returns:
        Tuple of (list of valid records, count of failed records)
    """
    records: List[AnnualAwardOut] = []
    failed_count = 0
    allowed_fields = set(AnnualAwardOut.model_fields.keys())
    ...
    for idx, row in df.iterrows():
        try:
            ...
            record = AnnualAwardOut(**row_dict)
            records.append(record)
        except ValidationError as exc:
            event_logger.bind(domain="annual_award", step="convert_to_models").debug(
                "Row validation failed", row_index=idx, error=str(exc)
            )
            failed_count += 1
        except Exception as exc:
            event_logger.bind(domain="annual_award", step="convert_to_models").warning(
...
def export_failed_records_csv(
    ...
    try:
        csv_path = export_error_csv(
            df,
            filename_prefix=f"failed_annual_award_{data_source}",
            output_dir=Path("logs"),
        )
        event_logger.bind(domain="annual_award", step="export_failed").info(
        ...
        event_logger.bind(domain="annual_award", step="export_failed").warning(
