...
CLEANSING_DOMAIN = "annual_award"
CLEANSING_REGISTRY = get_cleansing_registry()


class AnnualAwardIn(BaseModel):
    """Bronze layer input model - lenient validation.

    Handles both 企年受托中标 and 企年投资中标 sheets with unified schema.
    """
    ...
    中标日期: Optional[Union[date, str]] = Field(None, description="Award date")
...
class AnnualAwardOut(BaseModel):
    """Gold layer output model - strict validation.

    Unified output schema for both trustee and investee award records.
    """
    ...
    产品线代码: Optional[str] = Field(None, description="Product line code (QN01/QN02)")
    中标日期: Optional[date] = Field(None, description="Award date")
    ...
    @field_validator("中标日期", mode="before")
    @classmethod
    def parse_award_date(cls, v: Any) -> Optional[date]:
        """Parse award date to date."""
        if v is None or v == "":
            return None
        try:
            return parse_yyyymm_or_chinese(v)
...
class AnnualAwardProcessingResult(BaseModel):
    """Result container for annual award processing."""

    model_config = ConfigDict(validate_default=True, extra="forbid")

    records: List[AnnualAwardOut] = Field(
        ..., description="Processed annual award records"
    )
